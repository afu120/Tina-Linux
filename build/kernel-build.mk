...
define BuildKernel
  ...
  $(KERNEL_BUILD_DIR)/symtab.h: FORCE
	...
	if [ -e $(STAGING_DIR_ROOT)/lib/modules ]; then \
...
ifdef CONFIG_REDUCE_ROOTFS_SIZE
	...
	if [ "x$(TARGET_ARCH)" = "xriscv" ]; then \
		mv $(TARGET_DIR)/lib64xthead/lp64d/* $(TARGET_DIR)/lib/; \
		rm -rf $(TARGET_DIR)/lib64xthead/lp64d/; \
		ln -s ../lib $(TARGET_DIR)/lib64xthead/lp64d; \
		mv $(TARGET_DIR)/usr/lib64xthead/lp64d/* $(TARGET_DIR)/usr/lib/; \
		rm -rf $(TARGET_DIR)/usr/lib64xthead/lp64d/; \
		ln -s ../lib $(TARGET_DIR)/usr/lib64xthead/lp64d; \
	fi
  ...
  recovery_menuconfig: $(STAMP_PREPARED) $(STAMP_CHECKED) FORCE
	...
	@if [ ! -f "$(LICHEE_CHIP_CONFIG_DIR)/configs/$(subst $(TARGET_PLATFORM)-,,$(TARGET_BOARD))/linux/config-$(KERNEL_PATCHVER)_recovery" ]; then \
		$(LINUX_RECONF_DIFF_RECOVERY) $(LINUX_DIR)/.config > $(LINUX_RECONFIG_TARGET_RECOVERY); \
	else \
		cmp -s $(LINUX_DIR)/.config $(LINUX_RECONFIG_TARGET_RECOVERY) || { \
			cp -rf $(LINUX_DIR)/.config $(LINUX_DIR)/arch/$(LINUX_KARCH)/configs/tina_defconfig; \
			$(MAKE) CC=$(TARGET_CC) -C $(LINUX_DIR) ARCH=$(LINUX_KARCH) tina_defconfig > /dev/null; \
			cp $(LINUX_DIR)/.config $(LINUX_RECONFIG_TARGET_RECOVERY); \
			rm -rf $(LINUX_DIR)/arch/$(LINUX_KARCH)/configs/tina_defconfig; \
		} \
	fi
  ...
  ramfs_menuconfig: $(STAMP_PREPARED) $(STAMP_CHECKED) FORCE
	...
	$(_SINGLE)$(MAKE) -C $(LINUX_DIR) $(KERNEL_MAKEOPTS) menuconfig
	@if [ ! -f "$(LICHEE_CHIP_CONFIG_DIR)/configs/$(subst $(TARGET_PLATFORM)-,,$(TARGET_BOARD))/linux/config-$(KERNEL_PATCHVER)_ramfs" ]; then \
		$(LINUX_RECONF_DIFF_RAMFS) $(LINUX_DIR)/.config > $(LINUX_RECONFIG_TARGET_RAMFS); \
	else \
		cmp -s $(LINUX_DIR)/.config $(LINUX_RECONFIG_TARGET_RAMFS) || { \
			cp -rf $(LINUX_DIR)/.config $(LINUX_DIR)/arch/$(LINUX_KARCH)/configs/tina_defconfig; \
			$(MAKE) CC=$(TARGET_CC) -C $(LINUX_DIR) ARCH=$(LINUX_KARCH) tina_defconfig > /dev/null; \
			cp $(LINUX_DIR)/.config $(LINUX_RECONFIG_TARGET_RAMFS); \
			rm -rf $(LINUX_DIR)/arch/$(LINUX_KARCH)/configs/tina_defconfig; \
		} \
	fi
